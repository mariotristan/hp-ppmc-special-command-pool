  ...
  CREATE OR REPLACE PACKAGE "PPMC"."BETEO_KCRT_UTIL" is

  -- Author  : TOR_NEU<br>
  -- Created : 11.03.2004 10:59:43<br>
  -- Version : $Id: BETEO_kcrt_util.pck,v 7.1 2007/07/10 mavo Exp $<br>
  ...
  *  07/10/2008	 Markus von der Heiden added set_created_by()
  */
  ...
  *  Sets the assigned user on a request.
  *
  *  #param p_request_id  The ID of the request to get a new created user
  *  #param p_user_id     The ID of the newly assigned user.
  *
  *  #return Returns SUCCESS on successful completion, FAILURE otherwise.
  */
  function set_created_by(p_request_id kcrt_requests.request_id%type,
                             p_user_id    knta_users.user_id%type)
    return varchar2;
...
/*
  * Ampel für PMO 
  */
  function traffic_light(p_date_first date,p_date_second date, p_date_third date,p_date_fourth date) return varchar2;

end BETEO_KCRT_UTIL;
/
CREATE OR REPLACE PACKAGE BODY "PPMC"."BETEO_KCRT_UTIL" is
  ...
  */
  ...
  begin
    update kcrt_requests r
       set r.last_update_date=sysdate,
           r.last_updated_by=p_user_id,
           r.entity_last_update_date=sysdate,
           r.created_by=p_user_id
    where  r.request_id=p_request_id;
    update kcrt_request_details rd
       set rd.last_update_date=sysdate,
           rd.last_updated_by=p_user_id,
           rd.created_by=p_user_id
    where  rd.request_id=p_request_id;
    return 'SUCCESS';
  exception
    when others then
      dbms_output.put_line('ERROR: set_created_by() FAILED');
      dbms_output.put_line('ERROR: SQLCODE : ' || sqlcode);
      dbms_output.put_line('ERROR:  SQLMSG : ' || sqlerrm);
      return 'FAILURE';
  end set_created_by;
...
function traffic_light(p_date_first date,
                       p_date_second date,
                       p_date_third date,
                       p_date_fourth date)
                       return varchar2 is
  ...
  --p_date_third  date;
  --p_date_fourth date;
  l_image varchar2(2000);
  begin
    dbms_output.put_line(p_date_first);
    dbms_output.put_line(p_date_second);
    dbms_output.put_line(p_date_third);
    dbms_output.put_line(p_date_fourth);
    dbms_output.put_line(p_date_first-p_date_second);
    dbms_output.put_line(p_date_third-p_date_fourth);
    if (p_date_first-p_date_second >=0 AND p_date_third-p_date_fourth >=0)
        then
        l_image:='Green';
        else
            if (p_date_first-p_date_second <0 AND p_date_third-p_date_fourth >=0)
            ...
            l_image:='Yellow';
              ...
              if (p_date_third-p_date_fourth <0)
              then
              l_image:='Red';
              else
              l_image:='Green';
