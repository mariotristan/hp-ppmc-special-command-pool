...
mam_utils = MamUtils('BusinessRFC.py')
if mam_utils.isInfoEnabled():
    mam_utils.info('########## Start BusinessRFC')
#######################################################
...
related_request_id            = Framework.getDestinationAttribute('related_request_id')
oracle_id                     = Framework.getDestinationAttribute('oracle_id')
...
if mam_utils.isInfoEnabled():
    ...
    mam_utils.info('########## table_user: ',table_user)
    mam_utils.info('########## view_name: ',view_name)
    mam_utils.info('########## table_name: ',table_name)
    mam_utils.info('########## request_type_name: ',request_type_name)
    mam_utils.info('########## related_request_type_name: ',related_request_type_name)
    mam_utils.info('########## related_request_id: ',related_request_id)
    mam_utils.info('########## oracle_id: ',oracle_id)
...
####################################################### 
dbObj                         ="select request_id,description, created_by_username, status_name from mitg.kcrt_requests_v where request_type_name='SAPCCM CR Development Request'"
#dbLink                        ="select cr.request_id, cr.description, cr.created_by_username, cr.status_name, dr.request_id, dr.description from kcrt_requests_v cr, kcrt_requests_v dr where cr.request_type_name='SAPCCM CR Development Request' and instr(cr.parameter41,dr.request_id)>0"
...
def parseDbObjRes(dbObjRes):
        if dbObjRes:
            ...
            for row in range(rows):
                ...
                ####################################################
                dbObjOSH        = ObjectStateHolder('SAPCCM_CR_Development_Request', 6)
                ####################################################
...
#            if mam_utils.isDebugEnabled():
#                mam_utils.debug('########## sql-statement: ' ,dbLink)
#                mam_utils.debug('########## Number of Rows: ' , rows)
#                mam_utils.debug('########## Number of Columns: ' , cols)
#            for row in range(rows):
#                ###################################################
#                ### map discovered values to variables
#                ###################################################
#                parent_name           = dbLinkRes.getCell(row, 0)
#                child_name            = dbLinkRes.getCell(row, 4)
#                ####################################################
#                ### define object stateholder for discovered CI
#                ### ObjectStateHolder([CI-TYPE],6)
#                ####################################################
#                dbparentOSH        = ObjectStateHolder('SAPCCM_CR_Development_Request', 6)
#                dbchildOSH         = ObjectStateHolder('SAPCCM_CR', 6)
#                ####################################################
#                ### map variables to CI attributes 0=overwrite 1=insert
#                ####################################################
...
#                if mam_utils.isDebugEnabled():
#                    mam_utils.debug('########## found the following attributes: ' ,dbLink)
#                    mam_utils.debug('########## parent_name: ' ,parent_name)
#                    mam_utils.debug('########## child_name: ' ,child_name)
#                ####################################################
#                ### add link to CMDB
#                ####################################################
#                linkOSH = HostKeyUtil.getLink('IS_DEV_CR', dbparentOSH, dbchildOSH)
