...
def parseDbRelation(dbRelationRes):
		if dbRelationRes:
			...
			for row in range(rows):
				transactionID		= dbRelationRes.getCell(row, 0)
				##parentType=parentType.replace(' ','_')
				transactionName	= dbRelationRes.getCell(row, 1)
				programName	= dbRelationRes.getCell(row, 2)
				sap_programOSH=ObjectStateHolder('SAPCCM_ALM_Program',6)
				sap_programOSH.setAttribute(AttributeStateHolder('data_name', programName, 0, 'String'))
				mam_utils.debug('----- sap_programOSH ' , sap_programOSH)
				OSHVSResult.add(sap_programOSH)
