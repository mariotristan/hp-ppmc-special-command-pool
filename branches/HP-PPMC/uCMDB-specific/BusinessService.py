...
mam_utils = MamUtils('BusinessService.py')
if mam_utils.isInfoEnabled():
    mam_utils.info('########## Start BusinessService')
#######################################################
...
dbLink3                       ="select r.request_type_name, r.request_id, r.visible_parameter1, r.status_code from kcrt_requests_v r WHERE   r.request_type_name='%s' "%(request_type_name)
...
def discoverOracle(sqlAgent, ip, oracleOSH):
        ...
        dbLink3Res                 = doQuery(sqlAgent, dbLink3, ip)
        parseDbLink3Res(dbLink3Res)
